...
int main()
{
	...
	int current_time=0;
	int completed = 0;
	float sum_wt=0,sum_tat=0,sum_rt=0,total_idle_time=0,prev=0;
	float cpu_utilization;
	
	int max_completion_time,min_arrival_time,length_cycle;
	...
	while(completed!=n)
	{
		...
		{
			if(bt_remaining[min_index]== ps[min_index].bt) //first time in the CPU
			{
	...
	printf("\n maxxxx= %d",max_completion_time);
	printf("\n minnn= %d",min_arrival_time);
	for(int i=0;i<n;i++)
	{   printf("INSIDE FOR");
	...
	//printf("\n Length of cycle= %f",length_cycle);
	//Output
